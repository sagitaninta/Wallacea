...
prior_df["Prior"]=prior_df.apply(lambda row: list(row[4:]) , axis=1)
subset_prior=prior_df[["Variant_ID","Ref","Alt","sift","Prior"]]
...
if (likelihood_df.size>0):
	...
	likelihood_df["Variant_ID"]=likelihood_df.apply(lambda row: str(row.Chromosome) +"_" + str(row.Position), axis=1)
	...
	posterior_output=final_df[["Variant_ID", "Ref","Alt","sift"]].join(posterior_df)
else:
	posterior_output=pd.DataFrame(0, index=np.arange(0), columns=["Variant_ID","Ref","Alt","sift","AA","AC","AG","AT","CC","CG","CT","GG","GT","TT"])
